...
const NON_LATIN_TABLES =
  /transliterate\/(cyrillic|greek|arabic|armenian|georgian|dhivehi|hebrew|hangul|kana|devanagari|bengali|gurmukhi|gujarati|oriya|tamil|telugu|kannada|malayalam|locales-cyrillic|locales-other)\.ts$/
const TABLE_FILES = SOURCES.filter((f) =>
  /transliterate\/(latin|symbols|cyrillic|greek|arabic|armenian|georgian|dhivehi|hebrew|hangul|kana|devanagari|bengali|gurmukhi|gujarati|oriya|tamil|telugu|kannada|malayalam)\.ts$/.test(
    f,
  ),
)
...
describe("ratchets: invariants read from the source tree", () => {
  ...
  it("script table files are plain object literals with no side effects", () => {
    expect(TABLE_FILES.length).toBe(20)
