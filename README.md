...
| `iriToUri(iri, { bidi, nfc, strict })`                                                                             | 3987 §3.1                  | UTF-8 percent-encoding without touching the characters (§3.1 step 1c); `nfc: true` for text from non-Unicode sources; `strict: true` throws on characters no IRI may contain; bidi formatting characters throw (`bidi: "strip"` removes them)                                                                                                |
